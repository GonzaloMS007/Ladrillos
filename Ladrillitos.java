...
public class Ladrillitos extends Frame implements WindowListener,  MouseListener, MouseMotionListener, Runnable {
    ...
    //Creamos una nueva instancia de Ladrillos
    public Ladrillitos() {
    setTitle("Ladrillos - Gonzalo - ITSAT");
    setSize(520, 400); //le damos el tamaño a la pantalla
    //localizamos al centro nuestro programa
    setLocationRelativeTo(null);
    add(panel); //agregamos el panel
    panel.setBackground(Color.blue); //lo pintamos de azul
    panel.addMouseMotionListener(this); //le decimos que 
    ///escuche el evento de mover el mouse
    ...
    public void dibujar(Color [] colores){
        ...
        g.setColor(Color.green); //le damos color a la pelotita
        ...
        g.setColor(Color.black); //le damos el color el texto
        g.drawString("Ladrillitos", 10, 15);
        g.drawString("Vidas:", 275, 15);
        g.setColor(Color.orange);
        g.drawString("nm635542@gmail.com", 120, 15);
        ...
        switch(vidas){//muestra la cantidad de vida con unos rectangulitos arriba de la pantalla 
            ...
            case 4: g.drawString("Bienvenidos", 230, 200);
                    g.drawString("Presione el boton del derecho"
                        + "del mouse para comenzar", 100, 220);
        ...
        if (score==300){
        g.drawString("Felicidades Ganaste!!!", 230, 200); 
        g.drawString("Calculo que dabes ser el jugador mas feliz del MUNDO, jaja", 70, 220);
       ...
       for (int x=0; x!=30; x++){
        ...
        if (py>this.getHeight()){ //si es mas bajo que la barra
         ...
         //para que rebote en las paredes y el piso
            ...
            incX *= -1; //multiplicarlo por -1 al eje x hace que cambie 
            //totalmente el rumbo de la pelotita
            }
         if (py<20){
            incY *= -1; //multiplicarlo por -1 al eje y hace que cambie 
            //totalmente el rumbo de la pelotita
             }
       ...
       if ((py<74) && (py>56)){
        ...
        //para que la barra no salga del frame( de la ventana)
    ...
    while ((true)&&(score!=300)){
        this.mover(); //que nos sirve para el rebote de la pelota y 
        //para saber si paso sobre un ladrillito    
        this.dibujar(colores); //dibuja todo (seria bueno hacerlo por
        //parte para manejar mejor el tema de thread)
        try{
            Thread.sleep(50);
        }
    ...
    if (vidas==4){
        vidas--;//si las vidas son 4 cuando hace un click se le elimina uno
        //(lo de 4 solo es para mostar un mensaje)
    }
    if (vidas==0){
